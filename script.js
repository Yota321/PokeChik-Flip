class EnhancedFlipBook {
    constructor() {
        this.currentPage = 0;
        this.totalPages = 12; // 0-11 pages (24 sides total)
    ...
    flipPage(pageIndex, direction) {
        const page = document.querySelector(`.page-${pageIndex}`);
        if (!page) return;
    ...
    showThankYou() {
        this.thankYouOverlay.classList.add('active');
        this.controls.style.opacity = '0.5';
    }
    
    hideThankYou() {
        this.thankYouOverlay.classList.remove('active');
        this.controls.style.opacity = '1';
    }
    ...
    updateDisplay() {
        ...
        if (this.currentPage === 0) {
            ...
            const leftPage = (this.currentPage - 1) * 2 + 2;
        ...
        this.nextBtn.disabled = false; // Always allow next (for thank you page)
